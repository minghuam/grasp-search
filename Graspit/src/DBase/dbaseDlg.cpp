...
void DBaseDlg::renderOpenButton_clicked(){
    QString dir = QFileDialog::getExistingDirectory(this);
    this->renderPathEdit->setText(dir);
}

void DBaseDlg::renderButton_clicked(){
    ...
    viewer = graspItGUI->getIVmgr()->getViewer();
    renderArea = viewer;
    sg = renderArea->getSceneGraph();
    ...
    renderRoot->ref();
    renderRoot->addChild(viewer->getCamera());
    SoTransformSeparator *lightSep = new SoTransformSeparator;
    SoRotation *lightDir = new SoRotation;
    lightDir->rotation.connectFrom(&viewer->getCamera()->orientation);
    lightSep->addChild(lightDir);
    lightSep->addChild(viewer->getHeadlight());
    ...
    QString filename = QDir(renderPathEdit->text()).filePath("test.jpg");
